...
When a PR adds a post, CI commits an empty brief with the questions (`.github/workflows/social-brief.yml`). The author answers them, by hand or with any AI assistant (see Social Brief in the top-level `README.md`), and the `social-brief` check fails until every answer is filled in. To create it locally first, run `uv run scripts/social/create_brief.py posts/<file>`.
