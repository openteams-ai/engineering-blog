...
Christopher Farrow watched the conflict markers and saw something I had not phrased out loud. Sales, he pointed out, is in many ways an alignment problem. What the review document does is surface every point where alignment might be missing, from the customer side or from ours. Each flag is a numbered review requirement (`RR-##`), the review-side counterpart to the strategy meeting's `DR-##`, traceable back to a specific moment in the call. The flags are not just bookkeeping. They are a map of where the deal could go sideways.
